...
-- Cleanup old UI
for _, n in ipairs({"Aurora"}) do
    pcall(function() local o = gethui():FindFirstChild(n); if o then o:Destroy() end end)
...
local function collectIncome()
    -- CollectAllBrainrotIncome: single call replaces looping 19 slots
...
-- Auto Buy Gear (with auto-restock)
task.spawn(function()
    while alive() do
        ...
        pcall(function()
            ...
            -- Check if any selected gear is in stock
            local anyInStock = false
            for gearName, count in pairs(stock) do
                if SelGears[gearName] and count > 0 then anyInStock = true; break end
            end
            -- Auto-restock if nothing available
            if not anyInStock then
                pcall(function() Remotes.InstantRestock:InvokeServer() end)
                task.wait(0.5)
                ok, stock = pcall(function() return Remotes.GetShopStock:InvokeServer() end)
                if not ok or typeof(stock) ~= "table" then return end
            end
            -- Buy all selected gear
            for gearName, count in pairs(stock) do
                if not G().AutoBuyGear or not alive() then return end
                if not SelGears[gearName] then continue end
                if count <= 0 then continue end
                pcall(function() Remotes.PurchaseShopGear:InvokeServer(gearName) end)
                S.gears += 1
                task.wait(0.8 + math.random() * 0.2)
            end
        end)
    end
end)
...
-- Auto Collect Income (CollectAll = 1 call, ultra fast)
task.spawn(function()
    while alive() do
        task.wait(jitter(5, 1))
        if not alive() then break end
...
-- Inline checkbox row (not a popup) — since we have 4 distinct filter tables
-- and each gets its own checkbox list. Works inside panel scroll.
-- =========================================================================
