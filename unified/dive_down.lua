...
---------- NETWORKING (preserved verbatim from v4) ----------
...
do
    local okReq, mod = pcall(function()
        local folder = RS:FindFirstChild("Packets")
        local node = folder and folder:FindFirstChild("Packet")
        return node and require(node) or nil
    end)
    if okReq and mod then Packet = mod end
end
local PKT = {}
if Packet then
    pcall(function() PKT.SellInventory      = Packet("SellInventory") end)
    pcall(function() PKT.SellItem           = Packet("SellItem") end)
    pcall(function() PKT.ClaimOfflineReward = Packet("ClaimOfflineReward") end)
    pcall(function() PKT.BuyItem            = Packet("BuyItem") end)
    pcall(function() PKT.SpinWheel          = Packet("SpinWheel") end)
end

local function fireNet(name, arg1, arg2)
    pcall(function()
        local r = NET and NET:FindFirstChild(name .. "-RemoteEvent")
        if r then
...
local function invokeNet(name, arg1)
    local res
    pcall(function()
        local r = NET and NET:FindFirstChild(name .. "-RemoteFunction")
        if r then
...
---------- ZOMBIE KILL ----------
if getgenv().__AURORA_DIVE_CFG2 then
    for k, v in pairs(getgenv().__AURORA_DIVE_CFG2) do
        if type(v) == "boolean" then getgenv().__AURORA_DIVE_CFG2[k] = false end
    end
end
task.wait(0.15)

getgenv().__AURORA_DIVE_SESSION = tick()
local _mySession = getgenv().__AURORA_DIVE_SESSION
local function alive() return getgenv().__AURORA_DIVE_SESSION == _mySession end
...
---------- CONFIG ----------
if not getgenv().__AURORA_DIVE_CFG2 then
    getgenv().__AURORA_DIVE_CFG2 = {
        AutoFarm         = false,
        FarmAutoSell     = false,
        AutoSell         = false,
        AutoEquipBest    = false,
    ...
    local c = getgenv().__AURORA_DIVE_CFG2
    if c.ActiveTab == nil then c.ActiveTab = "Farm" end
    if c.PanelOpen == nil then c.PanelOpen = true end
    if c.ProtectMutations == nil then c.ProtectMutations = true end
end
local CFG = getgenv().__AURORA_DIVE_CFG2
...
local function saveCFG()
    ...
    pcall(function() if _HAS.writefile then writefile(_cfgFileName, HttpService:JSONEncode(toSave)) end end)
...
local function doSell()
    pcall(function() if PKT.SellInventory then PKT.SellInventory:Fire() end end)
    S.sells += 1
end
...
pcall(function()
    local notif1   = NET and NET:FindFirstChild("ShowNotification-RemoteEvent")
    local notif2   = NET and NET:FindFirstChild("ShowNotfication-RemoteEvent")
    ...
    for _, remote in ipairs({notif1, notif2}) do
        if remote then
...
local function doBuyTreats()
    ...
    for name, sel in pairs(SelTreats) do
        ...
        if sel and (stock.Treat[name] or 0) > 0 then
            pcall(function() if PKT.BuyItem then PKT.BuyItem:Fire("Treat", name) end end)
            S.treats += 1
            task.wait(1.5)
        end
    end
...
local function doBuyTools()
    ...
    for name, sel in pairs(SelTools) do
        ...
        if sel and (stock.Tool[name] or 0) > 0 then
            pcall(function() if PKT.BuyItem then PKT.BuyItem:Fire("Tool", name) end end)
            S.tools += 1
            task.wait(1.5)
        end
    end
...
local function doFeedFish()
    ...
    pcall(function()
        local remote = NET and NET:FindFirstChild("Get Save-RemoteFunction")
...
local function doClaim()
    ...
    pcall(function() if PKT.ClaimOfflineReward then PKT.ClaimOfflineReward:Fire() end end)
    fireNet("ClaimTreatPack")
    invokeNet("ClaimKrakenEgg")
end

local function doSpinWheel()
    pcall(function() if PKT.SpinWheel then PKT.SpinWheel:Fire() end end)
...
local function doClaimQuests()
    pcall(function()
        ...
        for questId, qData in pairs(quests) do
            ...
            if completed then
                pcall(function()
                    local r = NET and NET:FindFirstChild("ClaimQuest-RemoteFunction")
                    if r then r:InvokeServer(questId); S.quests = (S.quests or 0) + 1 end
                end)
                task.wait(0.3)
            end
        end
...
task.spawn(function() while alive() do if CFG.AutoSell      then pcall(doFilteredSell) end; task.wait(jitter(5, 1.5));  if not alive() then break end end end)
task.spawn(function() while alive() do if CFG.AutoEquipBest then pcall(doEquipBest)   end; task.wait(jitter(10, 3.0)); if not alive() then break end end end)
task.spawn(function() while alive() do if CFG.AutoBuyTreats then pcall(doBuyTreats)   end; task.wait(jitter(10, 3.0)); if not alive() then break end end end)
task.spawn(function() while alive() do if CFG.AutoBuyTools  then pcall(doBuyTools)    end; task.wait(jitter(10, 3.0)); if not alive() then break end end end)
...
task.spawn(function() while alive() do if CFG.AutoSpinWheel   then pcall(doSpinWheel)    end; task.wait(jitter(60, 15.0)); if not alive() then break end end end)
...
---------- AUTO-FARM DIVE LOOP (preserved verbatim from v4) ----------
...
local function getMyPlot()
    ...
    pcall(function()
        local rf = safeRF(NET, "GetPlayerPlot-RemoteFunction")
        local plot = rf and rf:InvokeServer() or nil
...
-- =========================================================================
-- DROPDOWN (operates on EXTERNAL set map — preserves v4's shared-state semantics
-- for SelRarities/SelMutations/SelZones/SelTreats/SelTools/SelFeedSlots/SelSellRarities)
-- =========================================================================
...
actionBtn(scrolls["Farm_alpha"], "TP to Ice Area",      C.bg3, nFa(), function()
    pcall(function()
        local hrp = getHRP()
        if not hrp then return end
...
actionBtn(scrolls["Settings_alpha"], "Destroy UI", C.red, nSeta(), function()
    task.wait(0.3)
    getgenv().__AURORA_DIVE_SESSION = 0
...
task.spawn(function()
    while alive() do
        ...
        pcall(function()
            ...
            elseif CFG.AutoRebirth or CFG.AutoClaim then mode = "Rewards"
