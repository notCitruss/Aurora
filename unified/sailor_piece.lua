...
--========================================================================
-- CLEANUP (Xeno-safe: each parent chain in its own pcall)
--========================================================================
for _, n in ipairs({"Aurora", "AuroraPill", "AuroraTestUI", "IconPreview", "AuroraESP"}) do
...
-- SESSION KILL + alive()
--========================================================================
if getgenv().__AURORA_SAILOR_CFG2 then
    for k, v in pairs(getgenv().__AURORA_SAILOR_CFG2) do
        if type(v) == "boolean" then getgenv().__AURORA_SAILOR_CFG2[k] = false end
    end
end
getgenv().__AURORA_SAILOR_SESSION = tick()
local _mySession = getgenv().__AURORA_SAILOR_SESSION
local function alive() return getgenv().__AURORA_SAILOR_SESSION == _mySession end
...
local function getRemote(name)
    ...
    for _, source in ipairs(sources) do
        local r = source:FindFirstChild(name)
...
-- InputHoldBegin + VIM E-key hold fallbacks for Xeno and others that lack the helper.
...
local function loadSavedCFG()
    ...
    if not saved then saved = getgenv()["AuroraCFG_sailor_piece"] end
...
local function saveCFG()
    ...
    getgenv()["AuroraCFG_sailor_piece"] = CFG
end

loadSavedCFG()

-- Keep getgenv CFG2 alias in sync so zombie-kill sweep reaches this instance
getgenv().__AURORA_SAILOR_CFG2 = CFG
...
-- If the scan returns empty (user loaded in Starter Island before mobs streamed in),
-- we fall back to a baked-in list of known mobs/bosses across the 17 islands.
...
-- Ensures dropdown is always complete even if user loads in starter island.
local _ALL_MOBS_BASELINE = {
    -- Normal mobs (5-of-each group, sorted alpha)
    ...
    -- Bosses (also in _BOSS_MOBS_BASELINE for the boss-only filter)
...
-- UNION scan: always starts from baseline, adds any live spawns we don't know about yet.
...
local function tpToIsland(island)
    -- 1. Try TeleportToPortal remote (game's proper teleport)
    pcall(function()
        local re = getRemote("TeleportToPortal")
        if re then re:FireServer(island.portalId) end
    end)
    -- 2. Also try TeleportToIslandSpot as backup
    pcall(function()
        local re = getRemote("TeleportToIslandSpot")
        if re then re:FireServer(island.portalId) end
    end)
    -- 3. CFrame fallback after short delay (in case remotes work with loading screen)
...
getgenv().__AURORA_SAILOR_COLLECTED_EGGS = getgenv().__AURORA_SAILOR_COLLECTED_EGGS or {}
local _collectedEggs = getgenv().__AURORA_SAILOR_COLLECTED_EGGS
task.spawn(function()
    while alive() do
                ...
                for _, t in ipairs(targets) do
                    ...
                    pcall(function()
                        ...
                        for attempt = 1, 4 do
                            ...
                            pcall(function()
                                if _HAS.firepp then fireproximityprompt(t.prompt) end
                            end)
                            pcall(function()
...
-- Currency filter: buys only "Gems" / "Money" / "Coins" items — skips anything else (Robux, tokens, etc.).
task.spawn(function()
    while alive() do
    ...
    while alive() do
        ...
        if ugActive() and not _ugConn then
            _ugConn = RunService.Stepped:Connect(function()
                if not alive() then if _ugConn then _ugConn:Disconnect(); _ugConn = nil end; return end
                        ...
                        if tick() - lastVisit >= 30 then
                            pcall(function()
                                ...
                                if bestNPC and bestRoot and bestPrompt then
                                    ...
                                    task.wait(0.3)
                                    pcall(function() safeFirePrompt(bestPrompt) end)
                                    task.wait(2)
                                    hrp.CFrame = savedPos
...
task.spawn(function()
    while alive() do
        task.wait(0.1)
        if not alive() then if _noclipConn then _noclipConn:Disconnect(); _noclipConn = nil end; break end
        if CFG.Noclip and not _noclipConn then
            _noclipConn = RunService.Stepped:Connect(function()
                if not alive() or not CFG.Noclip then if _noclipConn then _noclipConn:Disconnect(); _noclipConn = nil end; return end
                pcall(function()
                    local char = Player.Character
...
task.spawn(function()
    while alive() do
        ...
        if CFG.AutoStats then
            pcall(function()
                local rfGet = getRemote("GetPlayerStats")
                if not rfGet or not rfGet:IsA("RemoteFunction") then return end
                local ok, stats = pcall(function() return rfGet:InvokeServer() end)
                if ok and stats and stats.StatPoints and stats.StatPoints > 0 then
                    local re = getRemote("AllocateStats")
...
task.spawn(function()
    while alive() do
        ...
        if CFG.AutoBoss then
            pcall(function()
                local re = getRemote("RequestSummonBoss")
                if re then re:FireServer() end
            end)
        end
    end
...
-- ESP (weak-table refs, never destroy/recreate — update in place)
local _espFolder = Instance.new("Folder"); _espFolder.Name = "AuroraESP"
local _efOk = false
if _HAS.gethui then _efOk = pcall(function() _espFolder.Parent = gethui() end) end
if not _efOk then _efOk = pcall(function() _espFolder.Parent = game:GetService("CoreGui") end) end
if not _efOk then pcall(function() _espFolder.Parent = Player:WaitForChild("PlayerGui") end) end

-- Weak tables: auto-cleaned when target is GC'd
local _espBBs = setmetatable({}, {__mode = "k"})   -- [instance] = {bb, label}
local _espBoxes = setmetatable({}, {__mode = "k"})  -- [model] = SelectionBox
local _espHL = setmetatable({}, {__mode = "k"})     -- [model] = Highlight (skeleton outline)
...
task.spawn(function()
    while alive() do
        ...
        pcall(function()
            ...
            -- Skeleton ESP: Highlight with FillTransparency=1
...
actionBtn    (scrolls["Settings_alpha"], "Destroy UI", C.red, nS(), function()
    task.wait(0.3)
    getgenv().__AURORA_SAILOR_SESSION = 0
...
infoRow(scrolls["Settings_beta"], "Game",    "Sailor Piece",           C.text,  nSB())
...
--========================================================================
-- EXPLOIT LOOPS (preserved from v5 AH + Xeno-guarded)
--========================================================================

-- Zero Cooldown (getgc-based — Xeno-gated)
task.spawn(function()
    while alive() do
        task.wait(5)
        if not alive() then break end
        if CFG.ZeroCooldown and _HAS.getgc then
            pcall(function()
                local patched = 0
